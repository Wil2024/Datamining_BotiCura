...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("""
   © 2026 Desarollado por Wilton Torvisco - Data Mining Simulator """)
...
with tab2:
    ...
    if st.button("Ejecutar Clustering"):
        ...
        insight(
            "Los segmentos permiten diseñar "
            "estrategias diferenciadas de fidelización, "
            "retención y cross-selling."
        )

        download_btn(
            feat,
            "segmentacion_clientes.xlsx",
...
with tab3:
    ...
    st.markdown(
        '<div class="section-header">💬 Clasificación de Sentimiento</div>',
        unsafe_allow_html=True
    )
...
with tab4:
    ...
    if st.button("Ejecutar Apriori"):
        ...
        if rules.empty:
            ...
            insight(
                "Las reglas permiten diseñar "
                "promociones, combos y estrategias "
                "de cross-selling."
            )

            download_btn(
                top_rules,
                "reglas_asociacion.xlsx",
                "Descargar Reglas"
            )
...
5. ¿Qué clientes poseen mayor riesgo de churn?
